...
<?php if ($permission['edit']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/lunisolardates/import/') ?>','_self');"><?=lang("button_import")?></button>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/lunisolardates/edit/') ?>','_self');"><?=lang("button_add")?></button>
<?php } ?>
<?php if ($permission['rank']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/lunisolardates/rank/') ?>','_self');"><?=lang("button_rank")?></button>
<?php } ?>
<?php if ($permission['export']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/lunisolardates/export') ?>','_blank');"><?=lang("button_export")?></button>
<?php } ?>
<?php if ($error){ ?>
...
$(document).ready(function() {
    datatable = $('#record_table').DataTable( {
        ...
        "ajax": {
			"url": "<?php echo site_url('module/lunisolardates/getRecordAjax') ?>",
            "type": "POST"
		},
		"columnDefs": [
...
function deleteRecord(id){
	if(confirm("<?=lang("button_delete_msg")?>")){
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/lunisolardates/deleteRecordAjax') ?>',
			type: 'POST',
			data: {"id": id },
