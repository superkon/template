  ...
  <div class="tabMainWrapper">
    ...
    <div class="innerBox-lightGrey">
        ...
        <div class="formRowWrapper">
            ...
            <br/>
            <button type="button" class="btn btn-primary" onclick="window.open('<?php echo site_url('module/lunisolardates/downloadImportSample/') ?>','_self');"><?=lang("button_sample")?></button>
					</div>
				</div>
        </form>
      </div>
...
$(document).ready(function(){
  ...
  $("#csv").on("change", function(){
    ...
    if (formdata) {
      showLoading();
      $.ajax({
        url: "<?php echo site_url('module/lunisolardates/importExcel') ?>",
        type: "POST",
        data: formdata,
        processData: false,
        contentType: false,
  ...
  $(".import_save").bind("click", function(){
    if(confirm("<?=lang("button_import_msg")?>")){
  		showLoading();
  		$.ajax({
  			url: '<?php echo site_url('module/lunisolardates/importSave') ?>',
  			type: 'POST',
        dataType: "json",
  			data: {"data": csvdata },
