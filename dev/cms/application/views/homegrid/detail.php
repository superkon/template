<h2 class="sub-header"><?php echo $page_title; ?>
	<?php if ($show_version_button){ ?>
	  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/homegrid/version/'.$id) ?>','_self');"><?=lang("button_version")?></button>
	<?php } ?>
</h2>
...
<?php } ?>
			...
			<div class="btnWrapper aCenter wMarginTop">
				...
				<?php }?>
				<button class="last" type="button" name="button" onclick="window.open('<?php echo site_url('module/homegrid/listing') ?>','_self');"><?=lang("button_back")?></button>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function submitForm(submit_type) {
		...
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/homegrid/updateRecordAjax') ?>',
			type: 'POST',
          ...
          location.href = "<?php echo site_url('login') ?>";
					...
					if (data.redirect == "1"){
						location.href = "<?php echo site_url('module/homegrid/') ?>";
					}else{
						location.href = "<?php echo site_url('module/homegrid/edit/') ?>" + data.id;
					}
				}else{
