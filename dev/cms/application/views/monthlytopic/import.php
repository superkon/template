...
<button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/monthlytopic/listing/') ?>','_self');"><?=lang("button_back")?></button>
</h2>
  ...
  <div class="tabMainWrapper">
    ...
    <div class="innerBox-lightGrey">
        ...
        <div class="formRowWrapper">
            ...
            <br/>
            <button type="button" class="btn btn-primary" onclick="window.open('<?php echo site_url('module/monthlytopic/downloadImportSample/') ?>','_self');"><?=lang("button_sample")?></button>
					</div>
				</div>
        </form>
      </div>
...
$(document).ready(function(){
  ...
  $("#csv").on("change", function(){
    ...
    if (formdata) {
      showLoading();
      $.ajax({
        url: "<?php echo site_url('module/monthlytopic/importExcel') ?>",
        type: "POST",
        data: formdata,
        processData: false,
        contentType: false,
  ...
  $(".import_save").bind("click", function(){
    if(confirm("<?=lang("button_import_msg")?>")){
  		showLoading();
  		$.ajax({
  			url: '<?php echo site_url('module/monthlytopic/importSave') ?>',
  			type: 'POST',
        dataType: "json",
  			data: {"data": csvdata },
