...
<?php if ($permission['edit']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/contactus/edit/') ?>','_self');"><?=lang("button_add")?></button>
<?php } ?>
<?php if ($permission['rank']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/contactus/rank/') ?>','_self');"><?=lang("button_rank")?></button>
<?php } ?>
<?php if ($permission['export']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/contactus/export') ?>','_blank');"><?=lang("button_export")?></button>
<?php } ?>
<?php if ($error){ ?>
...
$(document).ready(function() {
    datatable = $('#record_table').DataTable( {
        ...
        "ajax": {
			"url": "<?php echo site_url('module/contactus/getRecordAjax') ?>",
            "type": "POST"
		},
		"columnDefs": [
...
function deleteRecord(id){
	if(confirm("<?=lang("button_delete_msg")?>")){
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/contactus/deleteRecordAjax') ?>',
			type: 'POST',
			data: {"id": id },
...
$(".sidebar-contactus a").addClass("selected");
var _parent = $(".sidebar-contactus").parents('.wSubMenu');
