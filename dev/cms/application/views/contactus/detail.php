<h2 class="sub-header"><?php echo $page_title; ?>
	<?php if ($show_version_button){ ?>
	  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/contactus/version/'.$id) ?>','_self');"><?=lang("button_version")?></button>
	<?php } ?>
</h2>
...
<script type="text/javascript">
	...
	function submitForm(submit_type) {
		...
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/contactus/updateRecordAjax') ?>',
			type: 'POST',
          ...
          location.href = "<?php echo site_url('login') ?>";
					...
					if (data.redirect == "1"){
						location.href = "<?php echo site_url('module/contactus/') ?>";
					}else{
						location.href = "<?php echo site_url('module/contactus/edit/') ?>" + data.id;
					}
				}else{
