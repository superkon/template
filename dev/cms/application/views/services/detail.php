<h2 class="sub-header"><?php echo $page_title; ?>
	<?php if ($show_version_button){ ?>
	  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/services/version/'.$id) ?>','_self');"><?=lang("button_version")?></button>
	<?php } ?>
</h2>
...
<?php } ?>
			...
			<div class="btnWrapper aCenter wMarginTop">
				...
				<?php }?>
				<button class="last" type="button" name="button" onclick="window.open('<?php echo site_url('module/services/listing') ?>','_self');"><?=lang("button_back")?></button>
			</div>
...
<script type="text/javascript">
	...
	function submitForm(submit_type) {
		...
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/services/updateRecordAjax') ?>',
			type: 'POST',
          ...
          location.href = "<?php echo site_url('login') ?>";
					...
					if (data.redirect == "1"){
						location.href = "<?php echo site_url('module/services/') ?>";
					}else{
						location.href = "<?php echo site_url('module/services/edit/') ?>" + data.id;
					}
				}else{
	...
	$(".sidebar-services a").addClass("selected");
	var _parent = $(".sidebar-services").parents('.wSubMenu');
