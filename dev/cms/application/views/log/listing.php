...
<?php if ($button['show_edit_button'] == "1"){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/log/edit') ?>','_self');"><?=lang("button_add")?></button>
<?php } ?>
<?php if ($button['show_export_button'] == "1"){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/log/export') ?>','_blank');"><?=lang("button_export")?></button>
<?php } ?>
<?php if ($error){ ?>
...
$(document).ready(function() {
    datatable = $('#record_table').DataTable( {
        ...
        "ajax": "<?php echo site_url('module/log/getRecordAjax') ?>",
		"columnDefs": [
...
function deleteRecord(id){
	if(confirm("<?=lang("button_delete_msg")?>")){
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/log/deleteRecordAjax') ?>',
			type: 'POST',
			data: {"id": id },
...
$(".sidebar-log a").addClass("selected");
var _parent = $(".sidebar-log").parents('.wSubMenu');
