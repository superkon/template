...
<?php if ($permission['edit']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/homebanner/edit/') ?>','_self');"><?=lang("button_add")?></button>
<?php } ?>
<?php if ($permission['rank']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/homebanner/rank/') ?>','_self');"><?=lang("button_rank")?></button>
<?php } ?>
<?php if ($permission['export']){ ?>
  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/homebanner/export') ?>','_blank');"><?=lang("button_export")?></button>
<?php } ?>
<?php if ($error){ ?>
...
$(document).ready(function() {
    datatable = $('#record_table').DataTable( {
        ...
        "ajax": {
			"url": "<?php echo site_url('module/homebanner/getRecordAjax') ?>",
            "type": "POST"
		},
		"columnDefs": [
...
function deleteRecord(id){
	if(confirm("<?=lang("button_delete_msg")?>")){
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/homebanner/deleteRecordAjax') ?>',
			type: 'POST',
			data: {"id": id },
...
$(".sidebar-homebanner a").addClass("selected");
var _parent = $(".sidebar-homebanner").parents('.wSubMenu');
