<h2 class="sub-header"><?php echo $page_title; ?>
	<?php if ($show_version_button){ ?>
	  <button type="button" class="btn btn-primary navbar-right" onclick="window.open('<?php echo site_url('module/dummy/version/'.$id) ?>','_self');"><?=lang("button_version")?></button>
	<?php } ?>
</h2>
...
<?php } ?>
			...
			<div class="btnWrapper aCenter wMarginTop">
				<?php if ($show_save_button){ ?>
					<button type="button" name="button" onclick="submitForm(0)">Save</button>
				<?php }?>
				<?php if ($show_request_button){ ?>
					<button type="button" name="button" onclick="submitForm(2)">Request for approval</button>
				<?php }?>
				<?php if ($show_approve_button){ ?>
					<button type="button" name="button" onclick="submitForm(3)">Approve</button>
				<?php }?>
				<?php if ($show_reject_button){ ?>
					<button type="button" name="button" onclick="submitForm(4)">Reject</button>
				<?php }?>
				<?php if ($show_cancelrequest_button){ ?>
					<button type="button" name="button" onclick="submitForm(5)">Cancel Request Approval</button>
				<?php }?>
				<?php if ($show_delete_button){ ?>
					<button type="button" name="button" onclick="submitForm(6)">Accept Delete</button>
				<?php }?>
				<?php if ($show_remove_button){ ?>
					<button type="button" name="button" onclick="submitForm(7)">Remove</button>
				<?php }?>
				<?php if ($show_cancelremove_button){ ?>
					<button type="button" name="button" onclick="submitForm(8)">Cancel Remove</button>
				<?php }?>
				<button class="last" type="button" name="button" onclick="window.open('<?php echo site_url('module/dummy/listing') ?>','_self');">Back</button>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function submitForm(submit_type) {
		...
		showLoading();
		$.ajax({
			url: '<?php echo site_url('module/dummy/updateRecordAjax') ?>',
			type: 'POST',
          ...
          location.href = "<?php echo site_url('login') ?>";
					...
					if (data.redirect == "1"){
						location.href = "<?php echo site_url('module/dummy/') ?>";
					}else{
						location.href = "<?php echo site_url('module/dummy/edit/') ?>" + data.id;
					}
				}else{
	...
	$(".sidebar-dummy a").addClass("selected");
	var _parent = $(".sidebar-dummy").parents('.wSubMenu');
