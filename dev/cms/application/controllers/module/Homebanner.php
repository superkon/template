...
class Homebanner extends Cms_Controller
{
  public function __construct()
  {
    ...
    $this->log_model->setModule($this->homebanner_model->getModuleName());

    //**handle permission *************************************************************************************************
    $permission = $this->homebanner_model->showButton();
  ...
  {
    redirect('module/homebanner/listing', 'refresh');
  }
  ...
  public function rank()
  {
    ...
    $this->content['fields'] = $this->homebanner_model->getFieldsListing();
    $this->content['page_title'] = $this->homebanner_model->getModuleDisplayName().' - '.lang('page_rank');

    $this->content['permission'] = $this->permission;
    //********************************************************************************************************************

    $this->page_content['content'] = $this->load->view('homebanner/rank', $this->content, true);
  ...
  public function getRecordRankAjax($catId = null)
  {
    ...
    //**Generating Query*************************************************
    $fields = array_keys($this->homebanner_model->getFieldsListing());

    $where = 'status < 10';

    $orderby = "rank desc";

    $result = $this->homebanner_model->get($fields, $where, $orderby);

    $resultTotal = $this->homebanner_model->getTotal();
    $result_arr['recordsTotal'] = $resultTotal;

    $resultTotal = $this->homebanner_model->getTotal($where);
    $result_arr['recordsFiltered'] = $resultTotal;
    //*********************************************************************

    //**Replace the Option value ******************************************
    $result_arr['data'] = $this->homebanner_model->getOptionValues($result, $options);
    //*********************************************************************

    //**Replace the Image to html code ******************************************
    $result_arr['data'] = $this->homebanner_model->getInputTypeHTML($result_arr['data'], 100);
    //*********************************************************************
  ...
  public function updateRecordRankAjax()
  {
    ...
    foreach($data as $idx=>$id){
      ...
      $this->homebanner_model->set($params, $id);
    }

    //Handle Approval ***********************************************************************************************
    //**********************************************************************************************************
    if ($this->homebanner_model->haveApproval()) {
      $this->homebanner_model->syncRank2Live();
    }
    //End Handle Approval ***********************************************************************************************
    //**********************************************************************************************************
  ...
  public function version($id)
  {
    ...
    $this->homebanner_model->getFieldsListing();
    $this->content['page_title'] = $this->homebanner_model->getModuleDisplayName().' - '.lang('page_version');

    $this->content['permission'] = $this->permission;
    $this->content['fields']['actionbtn'] = '';
    //********************************************************************************************************************
    $this->content['id'] = $id;
    $this->page_content['content'] = $this->load->view('homebanner/version', $this->content, true);
  ...
  public function getRecordVersionAjax($id)
  {
    ...
    //**Generating Query*************************************************
    $result = $this->homebanner_model->getVersionList($id);
    ...
    foreach ($result_arr['data'] as $idx => $val) {
      ...
      $result_arr['data'][$idx]['actionbtn'] .= parent::buildButton(lang("button_version"), 'window.open(\''.site_url('module/homebanner/edit/'.$val['record_id']."/".$val['version']).'\',\'_self\');', 'btn-success');
    }
  ...
  public function listing()
  {
    ...
    $this->content['fields'] = $this->homebanner_model->getFieldsListing();
    $this->content['page_title'] = $this->homebanner_model->getModuleDisplayName().' - '.lang('page_listing');

    $this->content['permission'] = $this->permission;
    $this->content['fields']['action'] = '';
    //********************************************************************************************************************

    $this->page_content['content'] = $this->load->view('homebanner/listing', $this->content, true);
  ...
  public function getRecordAjax()
  {
    ...
    if (trim($search['value'] != '')) {
      $search_fields = array_keys($this->homebanner_model->getFieldsSearch());
    ...
    $orderby = $this->homebanner_model->getTableSorting();
    if (array_key_exists($order[0]['column'], $fields)) {
      $orderby = $fields[$order[0]['column']].' '.$order[0]['dir'];
    }

    $result = $this->homebanner_model->get($fields, $where, $orderby, $start, $length);
    ...
    foreach ($result_arr['data'] as $idx => $val) {
      ...
      if ($this->permission['copy']) {
        $result_arr['data'][$idx]['action'] .= parent::buildButton(lang("button_copy"), 'window.open(\''.site_url('module/homebanner/copy/'.$val['id']).'\',\'_self\');', 'btn-success');
      }

      if ($this->permission['edit']) {
        $result_arr['data'][$idx]['action'] .= parent::buildButton(lang("button_edit"), 'window.open(\''.site_url('module/homebanner/edit/'.$val['id']).'\',\'_self\');', 'btn-success');
      }
  ...
  public function copy($id = null)
  {
    if ($id != null) {
      ...
      redirect('module/homebanner/edit');
    } else {
      $this->session->set_flashdata('error', 'Invalid Access');
      redirect('module/homebanner/listing');
    }
  }
  ...
  public function edit($id = null,  $version_id = null)
  {
    ...
    if ($id) {
      ...
      $this->content['page_title'] = $this->homebanner_model->getModuleDisplayName().' - '.lang('page_edit');

      $this->content['id'] = $id;

      $fields = null;
      $where = array('id' => $id, 'status <' => 10);
      $result = $this->homebanner_model->get($fields, $where);

      if (count($result) == 1) {
        $vsoloop_record = $result[0];

        if ($version_id != null){
          $version_record = $this->homebanner_model->getVersionId($id, $version_id);

          if ( count($version_record) > 0 ){
            if ($version_record[0]['data'] != ""){
              $version_data = json_decode($version_record[0]['data'] ,true);
              $section_fields = $this->homebanner_model->getFieldsEdit();
      ...
      $this->content['page_title'] = $this->homebanner_model->getModuleDisplayName().' - '.lang('page_add');
      $this->content['id'] = '';
    ...
    foreach($this->homebanner_model->getLang() as $lang){
    ...
    foreach($section_fields as $field_name => $field_obj){
      ...
      // check tab location*****************************************
      if (in_array($field_obj['field_name'], $this->homebanner_model->getApprovalFields())){
        $edit_form_content[lang('tab_approval')][0][] = $content;
      }else if (in_array($field_obj['field_name'], $this->homebanner_model->getCommonFields())){
        ...
        if (count($this->homebanner_model->getLang()) > 0){
    ...
    foreach($this->homebanner_model->getLang() as $lang){
      foreach ($fields_for_copy as $key => $fields){
        $edit_form_content[$lang['displayname']][$key][] = $this->buildCopyLangRow($lang['name'], $this->homebanner_model->getLang(), $key);
    ...
    $this->page_content['content'] = $this->load->view('homebanner/detail', $this->content, true);
  ...
  public function updateRecordAjax()
  {
    ...
    if (($formdata['submit_type'] == 0) || ($formdata['submit_type'] == 3) || ($formdata['submit_type'] == 4)) {
      ...
      //$fields = $this->homebanner_model->getFieldsEdit();

      $msg = '';

      $params = array();

      $msg = $this->homebanner_model->validateFields($formdata, $params);
      ...
      */
      ...
      if ($formdata['id'] == '') {
        ...
        $result = $this->homebanner_model->set($params);
        $id = $this->homebanner_model->getInsertId();
        ...
        if ($result) {
          ...
          $params = array();
          $params['rank'] = $id;
          $result = $this->homebanner_model->set($params, $id);
        } else {
          $result_arr['status'] = '0';
          $result_arr['msg'] = $this->homebanner_model->getModuleDisplayName().': System Error (101)';
        ...
        $result = $this->homebanner_model->set($params, $formdata['id']);
      ...
      if (!$result) {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->homebanner_model->getModuleDisplayName().': System Error (102)';
    ...
    if ($this->homebanner_model->haveApproval()) {
      ...
      $result = $this->homebanner_model->set($params, $result_arr['id']);

      if (!$result) {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->homebanner_model->getModuleDisplayName().': System Error (103) - Submit Type: '.$formdata['submit_type'];
      ...
      if ($formdata['submit_type'] == 3 || $formdata['submit_type'] == 6) {
        $this->homebanner_model->sync2Live($result_arr['id']);
        ...
        //** add Version *****************************************************************************************************
        $this->homebanner_model->addVersion($result_arr['id'], "Published");
      }
  ...
  public function deleteRecordAjax()
  {
    ...
    if ($result) {
      ...
      if ($this->homebanner_model->haveApproval()) {
        $this->homebanner_model->sync2Live($id);
      }
      ...
      $result_arr['status'] = '0';
      $result_arr['msg'] = $this->homebanner_model->getModuleDisplayName().': System Error (103)';
  ...
  public function export()
  {
    ...
    //add title row
    $titles = array_values($this->homebanner_model->getFieldsExport());
    $data1[] = $titles;

    $fields = array_keys($this->homebanner_model->getFieldsExport());
    ...
    $result_arr = $this->homebanner_model->getOptionValues($result, $options);
    ...
    $writer->writeSheet($data1, $this->homebanner_model->getModuleName());

    $filename = $this->homebanner_model->getModuleName().'-'.date('YmdHis').'.xlsx';
  ...
  {
    $this->homebanner_model->updateTableConfig();
  }
}
