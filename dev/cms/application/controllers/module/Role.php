...
class Role extends Cms_Controller
{
  public function __construct()
  {
    ...
    $this->log_model->setModule($this->role_model->getModuleName());

    //**handle permission *************************************************************************************************
    $permission = $this->role_model->showButton();
  ...
  {
    redirect('module/role/listing', 'refresh');
  }
  ...
  public function listing()
  {
    ...
    $this->content['fields'] = $this->role_model->getFieldsListing();
    $this->content['page_title'] = $this->role_model->getModuleDisplayName().' - '.lang('page_listing');

    $this->content['permission'] = $this->permission;
    $this->content['fields']['action'] = '';
    //********************************************************************************************************************

    $this->page_content['content'] = $this->load->view('role/listing', $this->content, true);
  ...
  public function getRecordAjax()
  {
    ...
    //**Generating Query*************************************************
    $fields = array_keys($this->role_model->getFieldsListing());

    $where = 'status < 10';

    $like = array();
    if (trim($search['value'] != '')) {
      $search_fields = array_keys($this->role_model->getFieldsSearch());
    ...
    $orderby = $this->role_model->getTableSorting();
    if (array_key_exists($order[0]['column'], $fields)) {
      $orderby = $fields[$order[0]['column']].' '.$order[0]['dir'];
    }

    $result = $this->role_model->get($fields, $where, $orderby, $start, $length);

    $resultTotal = $this->role_model->getTotal();
    $result_arr['recordsTotal'] = $resultTotal;

    $resultTotal = $this->role_model->getTotal($where);
    $result_arr['recordsFiltered'] = $resultTotal;
    //*********************************************************************

    //**Replace the Option value ******************************************
    $result_arr['data'] = $this->role_model->getOptionValues($result, $options);
    //*********************************************************************

    //**Replace the Image to html code ******************************************
    $result_arr['data'] = $this->role_model->getInputTypeHTML($result_arr['data'], 100);
    //*********************************************************************
    ...
    foreach ($result_arr['data'] as $idx => $val) {
      ...
      if ($this->permission['copy']) {
        $result_arr['data'][$idx]['action'] .= parent::buildButton(lang("button_copy"), 'window.open(\''.site_url('module/role/copy/'.$val['id']).'\',\'_self\');', 'btn-success');
      }

      if ($this->permission['edit']) {
        $result_arr['data'][$idx]['action'] .= parent::buildButton(lang("button_edit"), 'window.open(\''.site_url('module/role/edit/'.$val['id']).'\',\'_self\');', 'btn-success');
      }
  ...
  public function copy($id = null)
  {
    if ($id != null) {
      ...
      redirect('module/role/edit');
    } else {
      $this->session->set_flashdata('error', 'Invalid Access');
      redirect('module/role/listing');
    }
  }
  ...
  public function edit($id = null)
  {
    ...
    if ($id) {
      ...
      $this->content['page_title'] = $this->role_model->getModuleDisplayName().' - '.lang('page_edit');

      $this->content['id'] = $id;

      $fields = null;
      $where = array('id' => $id, 'status <' => 10);
      $result = $this->role_model->get($fields, $where);
      ...
      $this->content['page_title'] = $this->role_model->getModuleDisplayName().' - '.lang('page_add');
      $this->content['id'] = '';
    ...
    $section_fields = $this->role_model->getFieldsEdit();
    ...
    foreach($this->role_model->getLang() as $lang){
    ...
    foreach($section_fields as $field_name => $field_obj){
      ...
      // check tab location*****************************************
      if (in_array($field_obj['field_name'], $this->role_model->getApprovalFields())){
        $edit_form_content[lang('tab_approval')][0][] = $content;
      }else if (in_array($field_obj['field_name'], $this->role_model->getCommonFields())){
        $edit_form_content[lang('tab_info')][0][] = $content;
      }else{
        if (count($this->role_model->getLang()) > 0){
    ...
    foreach($this->role_model->getLang() as $lang){
      foreach ($fields_for_copy as $key => $fields){
        $edit_form_content[$lang['displayname']][$key][] = $this->buildCopyLangRow($lang['name'], $this->role_model->getLang(), $key);
    ...
    if ($this->role_model->haveApproval()) {
    ...
    $this->page_content['content'] = $this->load->view('role/detail', $this->content, true);
  ...
  public function updateRecordAjax()
  {
    ...
    if (($formdata['submit_type'] == 0) || ($formdata['submit_type'] == 3) || ($formdata['submit_type'] == 4)) {
      ...
      $fields = $this->role_model->getFieldsEdit();
      ...
      if ($formdata['id'] == '') {
        ...
        $result = $this->role_model->set($params);
        $id = $this->role_model->getInsertId();
        ...
        if ($result) {
          ...
          $params = array();
          $params['rank'] = $id;
          $result = $this->role_model->set($params, $id);
        } else {
          $result_arr['status'] = '0';
          $result_arr['msg'] = $this->role_model->getModuleDisplayName().': System Error (101)';
        ...
        $result = $this->role_model->set($params, $formdata['id']);
      ...
      if (!$result) {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->role_model->getModuleDisplayName().': System Error (102)';
    ...
    if ($this->role_model->haveApproval()) {
      ...
      $result = $this->role_model->set($params, $result_arr['id']);

      if (!$result) {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->role_model->getModuleDisplayName().': System Error (103) - Submit Type: '.$formdata['submit_type'];
      ...
      if ($formdata['submit_type'] == 3 || $formdata['submit_type'] == 6) {
        $this->role_model->sync2Live($result_arr['id']);
  ...
  public function deleteRecordAjax()
  {
    ...
    if ($result) {
      ...
      $result_arr['status'] = '0';
      $result_arr['msg'] = $this->role_model->getModuleDisplayName().': System Error (103)';
  ...
  public function export()
  {
    ...
    //add title row
    $titles = array_values($this->role_model->getFieldsExport());
    $data1[] = $titles;

    $fields = array_keys($this->role_model->getFieldsExport());

    $where = array('status <' => 10);
    $orderby = $this->role_model->getTableSorting();

    $result = $this->role_model->get($fields, $where, $orderby);

    $result_arr = $this->role_model->getOptionValues($result, $options);
    ...
    $writer->writeSheet($data1, $this->role_model->getModuleName());

    $filename = $this->role_model->getModuleName().'-'.date('YmdHis').'.xlsx';
  ...
  {
    $this->role_model->updateTableConfig();
  }
}
