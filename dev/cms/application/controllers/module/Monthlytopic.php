...
class Monthlytopic extends Cms_Controller
{
  public function __construct()
  {
    parent::__construct();
    $this->load->model('monthlytopic_model');
    $this->load->model('user_model');


    $this->load->model('log_model');
    $this->log_model->setModule($this->monthlytopic_model->getModuleName());

    //**handle permission *************************************************************************************************
    $permission = $this->monthlytopic_model->showButton();
  ...
  {
    redirect('module/monthlytopic/listing', 'refresh');
  }
  ...
  public function rank()
  {
    ...
    $this->content['fields'] = $this->monthlytopic_model->getFieldsListing();
    $this->content['page_title'] = $this->monthlytopic_model->getModuleDisplayName().' - '.lang('page_rank');

    $this->content['permission'] = $this->permission;
    //********************************************************************************************************************

    $this->page_content['content'] = $this->load->view('monthlytopic/rank', $this->content, true);
  ...
  public function getRecordRankAjax($catId = null)
  {
    ...
    //**Generating Query*************************************************
    $fields = array_keys($this->monthlytopic_model->getFieldsListing());

    $where = 'status < 10';

    $orderby = "rank desc";

    $result = $this->monthlytopic_model->get($fields, $where, $orderby);

    $resultTotal = $this->monthlytopic_model->getTotal();
    $result_arr['recordsTotal'] = $resultTotal;

    $resultTotal = $this->monthlytopic_model->getTotal($where);
    $result_arr['recordsFiltered'] = $resultTotal;
    //*********************************************************************

    //**Replace the Option value ******************************************
    $result_arr['data'] = $this->monthlytopic_model->getOptionValues($result, $options);
    //*********************************************************************

    //**Replace the Image to html code ******************************************
    $result_arr['data'] = $this->monthlytopic_model->getInputTypeHTML($result_arr['data'], 100);
    //*********************************************************************
  ...
  public function updateRecordRankAjax()
  {
    ...
    foreach($data as $idx=>$id){
      ...
      $this->monthlytopic_model->set($params, $id);
    }

    //Handle Approval ***********************************************************************************************
    //**********************************************************************************************************
    if ($this->monthlytopic_model->haveApproval()) {
      $this->monthlytopic_model->syncRank2Live();
    }
    //End Handle Approval ***********************************************************************************************
    //**********************************************************************************************************
  ...
  public function version($id)
  {
    ...
    $this->monthlytopic_model->getFieldsListing();
    $this->content['page_title'] = $this->monthlytopic_model->getModuleDisplayName().' - '.lang('page_version');

    $this->content['permission'] = $this->permission;
    $this->content['fields']['actionbtn'] = '';
    //********************************************************************************************************************
    $this->content['id'] = $id;
    $this->page_content['content'] = $this->load->view('monthlytopic/version', $this->content, true);
  ...
  public function getRecordVersionAjax($id)
  {
    ...
    //**Generating Query*************************************************
    $result = $this->monthlytopic_model->getVersionList($id);
    ...
    foreach ($result_arr['data'] as $idx => $val) {
      ...
      $result_arr['data'][$idx]['actionbtn'] .= parent::buildButton(lang("button_version"), 'window.open(\''.site_url('module/monthlytopic/edit/'.$val['record_id']."/".$val['version']).'\',\'_self\');', 'btn-success');
    }
  ...
  public function import(){
    ...
    $this->content['page_title'] = $this->monthlytopic_model->getModuleDisplayName().' - '.lang('page_import');

    //********************************************************************************************************************
    $this->page_content['content'] = $this->load->view('monthlytopic/import', $this->content, true);
  ...
  public function downloadImportSample(){
    ...
    $fields = $this->monthlytopic_model->getFieldsEdit();
    ...
    $writer->writeSheet($data1, $this->monthlytopic_model->getModuleName());

    $filename = $this->monthlytopic_model->getModuleName().'-Sample.xlsx';
  ...
  function importExcel(){
    ...
    if($this->upload->do_upload("csv"))
    {
      ...
      foreach($data_sheet1 as $index=>$row){
        ...
        $msg = $fields = $this->monthlytopic_model->validateFields($formdata, $params);
        if ($msg != ""){
  ...
  public function importSave(){
    ...
    foreach ($params_list as $params){
      ...
      $result = $this->monthlytopic_model->set($params);
      $id = $this->monthlytopic_model->getInsertId();
      ...
      if ($result) {
        ...
        $params = array();
        $params['rank'] = $id;
        $result = $this->monthlytopic_model->set($params, $id);
      } else {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->monthlytopic_model->getModuleDisplayName().': System Error (101)';
  ...
  public function listing()
  {
    ...
    $this->content['fields'] = $this->monthlytopic_model->getFieldsListing();
    $this->content['page_title'] = $this->monthlytopic_model->getModuleDisplayName().' - '.lang('page_listing');

    $this->content['permission'] = $this->permission;
    $this->content['fields']['action'] = '';
    //********************************************************************************************************************

    $this->page_content['content'] = $this->load->view('monthlytopic/listing', $this->content, true);
  ...
  public function getRecordAjax()
  {
    ...
    if (trim($search['value'] != '')) {
      $search_fields = array_keys($this->monthlytopic_model->getFieldsSearch());
    ...
    $orderby = $this->monthlytopic_model->getTableSorting();
    if (array_key_exists($order[0]['column'], $fields)) {
      $orderby = $fields[$order[0]['column']].' '.$order[0]['dir'];
    }

    $result = $this->monthlytopic_model->get($fields, $where, $orderby, $start, $length);
    ...
    foreach ($result_arr['data'] as $idx => $val) {
      ...
      if ($this->permission['copy']) {
        $result_arr['data'][$idx]['action'] .= parent::buildButton(lang("button_copy"), 'window.open(\''.site_url('module/monthlytopic/copy/'.$val['id']).'\',\'_self\');', 'btn-success');
      }

      if ($this->permission['edit']) {
        $result_arr['data'][$idx]['action'] .= parent::buildButton(lang("button_edit"), 'window.open(\''.site_url('module/monthlytopic/edit/'.$val['id']).'\',\'_self\');', 'btn-success');
      }
  ...
  public function copy($id = null)
  {
    if ($id != null) {
      ...
      redirect('module/monthlytopic/edit');
    } else {
      $this->session->set_flashdata('error', 'Invalid Access');
      redirect('module/monthlytopic/listing');
    }
  }
  ...
  public function edit($id = null,  $version_id = null)
  {
    ...
    if ($id) {
      ...
      $this->content['page_title'] = $this->monthlytopic_model->getModuleDisplayName().' - '.lang('page_edit');

      $this->content['id'] = $id;

      $fields = null;
      $where = array('id' => $id, 'status <' => 10);
      $result = $this->monthlytopic_model->get($fields, $where);

      if (count($result) == 1) {
        $vsoloop_record = $result[0];

        if ($version_id != null){
          $version_record = $this->monthlytopic_model->getVersionId($id, $version_id);

          if ( count($version_record) > 0 ){
            if ($version_record[0]['data'] != ""){
              $version_data = json_decode($version_record[0]['data'] ,true);
              $section_fields = $this->monthlytopic_model->getFieldsEdit();
      ...
      $this->content['page_title'] = $this->monthlytopic_model->getModuleDisplayName().' - '.lang('page_add');
      $this->content['id'] = '';
    ...
    foreach($this->monthlytopic_model->getLang() as $lang){
    ...
    foreach($section_fields as $field_name => $field_obj){
      ...
      // check tab location*****************************************
      if ($content != ""){
        if (in_array($field_obj['field_name'], $this->monthlytopic_model->getApprovalFields())){
          $edit_form_content[lang('tab_approval')][0][] = $content;
        }else if (in_array($field_obj['field_name'], $this->monthlytopic_model->getCommonFields())){
          ...
          if (count($this->monthlytopic_model->getLang()) > 0){
    ...
    foreach($this->monthlytopic_model->getLang() as $lang){
      foreach ($fields_for_copy as $key => $fields){
        $edit_form_content[$lang['displayname']][$key][] = $this->buildCopyLangRow($lang['name'], $this->monthlytopic_model->getLang(), $key);
    ...
    $this->page_content['content'] = $this->load->view('monthlytopic/detail', $this->content, true);
  ...
  public function updateRecordAjax()
  {
    ...
    if (($formdata['submit_type'] == 0) || ($formdata['submit_type'] == 3) || ($formdata['submit_type'] == 4)) {
      ...
      $fields = null;
      $where = array('month' => $formdata["month"], 'year ' => $formdata["year"], 'status <' => 10 );
      if ($formdata['id'] != '') {
      ...
      if ($formdata['id'] == '') {
        ...
        $result = $this->monthlytopic_model->set($params, $formdata['id']);
      ...
      if (!$result) {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->monthlytopic_model->getModuleDisplayName().': System Error (102)';
    ...
    if ($this->monthlytopic_model->haveApproval()) {
      ...
      $result = $this->monthlytopic_model->set($params, $result_arr['id']);

      if (!$result) {
        $result_arr['status'] = '0';
        $result_arr['msg'] = $this->monthlytopic_model->getModuleDisplayName().': System Error (103) - Submit Type: '.$formdata['submit_type'];
      ...
      if ($formdata['submit_type'] == 3 || $formdata['submit_type'] == 6) {
        $this->monthlytopic_model->sync2Live($result_arr['id']);
        ...
        //** add Version *****************************************************************************************************
        $this->monthlytopic_model->addVersion($result_arr['id'], "Published");
      }
  ...
  public function deleteRecordAjax()
  {
    ...
    if ($result) {
      ...
      if ($this->monthlytopic_model->haveApproval()) {
        $this->monthlytopic_model->sync2Live($id);
      }
      ...
      $result_arr['status'] = '0';
      $result_arr['msg'] = $this->monthlytopic_model->getModuleDisplayName().': System Error (103)';
  ...
  public function export()
  {
    ...
    //add title row
    $titles = array_values($this->monthlytopic_model->getFieldsExport());
    $data1[] = $titles;

    $fields = array_keys($this->monthlytopic_model->getFieldsExport());
    ...
    $result_arr = $this->monthlytopic_model->getOptionValues($result, $options);
    ...
    $filename = $this->monthlytopic_model->getModuleName().'-'.date('YmdHis').'.xlsx';
  ...
  {
    $this->monthlytopic_model->updateTableConfig();
  }
}
