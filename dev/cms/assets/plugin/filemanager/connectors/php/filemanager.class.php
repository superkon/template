...
class Filemanager {
	...
	public function __construct($extraConfig = '') {
		...
		$subfolder = substr($_SERVER["REQUEST_URI"], 0, strpos($_SERVER["REQUEST_URI"], "/plugin"));
		$content = file_get_contents("http://".$_SERVER['HTTP_HOST'].$subfolder."/filemanager.php");
...
private function is_valid_path($path) {
  ...
  // @see http://stackoverflow.com/questions/5642785/php-a-good-way-to-universalize-paths-across-oss-slash-directions
	...
	$substrpath = substr(realpath($path) . DIRECTORY_SEPARATOR, 0, strlen(realpath($this->path_to_files))) . DIRECTORY_SEPARATOR;
